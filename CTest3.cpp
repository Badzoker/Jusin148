...
void CTest3::Push_front(int _iValue)
{
	...
	while (nullptr != pSour->pPrevious_Index) //처음으로 땡기는 과정
	{
...
void CTest3::Push_back(int _iValue)
{
	...
	pTemp->iValue = _iValue;
	while (nullptr != pSour->pNext_Index)//마지막으로 미는 과정
	{
...
void CTest3::Pop(int _iValue)
{
	...
	while (_iValue != pSour->iValue)//마지막으로 미는 과정
	{
		...
		{
			cout << _iValue << " 값이 리스트에 없음" << endl;
			return;
		}
	}
	cout << pSour->iValue << "값 찾음" << endl;
	if (nullptr != pSour->pNext_Index)
	{
